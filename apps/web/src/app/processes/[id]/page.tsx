...
export default function ProcessDetailPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      getConversation(processId),
    ])
      .then(([loadedProcess, loadedDocuments, conversation]) => {
      ...
      .catch((reason: Error) => {
        ...
        if (/401|403|Sessão/i.test(reason.message)) router.replace("/login");
      })
    ...
    try {
      setViewer({
        ...
        url: URL.createObjectURL(
          await fetchDocumentBlob(processId, document.id),
        ),
      });
    } catch (reason) {
      setError(
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const uploaded = await uploadDocuments(process.id, files);
  ...
  async function handleDelete(document: DocumentSummary) {
    ...
    try {
      await deleteDocument(process.id, document.id);
  ...
  async function handleSend(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await sendMessage(process.id, content);
  ...
  async function handleClear() {
    if (
      ...
      return;
    try {
      await clearConversation(process.id);
      setMessages([]);
    } catch (reason) {
      setError(
        reason instanceof Error
    ...
    try {
      setProcess(
        process.status === "CONCLUIDO"
          ? await reopenProcess(process.id)
          : await completeProcess(process.id),
      );
    } catch (reason) {
      setError(
        reason instanceof Error
    ...
    <main className="app-shell">
      <aside className="sidebar" aria-label="Navegação principal">
        ...
        <nav className="navigation-list">
          ...
          <Link className="navigation-item" href="/documents">
      ...
      <section className="content-area">
        ...
        <section className="documents-panel process-detail-section">
          <div className="documents-heading">
            <div>
              <span className="eyebrow accent">DOCUMENTOS</span>
              <h2>Arquivos deste processo</h2>
            </div>
          ...
          <form className="upload-box" onSubmit={handleUpload}>
            ...
            <p>Somente PDF, até 20 MB por arquivo.</p>
            <label className="upload-button">
              {isUploading ? "Enviando..." : "Selecionar PDFs"}
              <input
                accept="application/pdf,.pdf"
                disabled={isUploading || process.status === "CONCLUIDO"}
                multiple
                name="documents"
                onChange={(event) => {
        ...
        <section className="chat-panel process-detail-section">
          <div className="documents-heading">
            <div>
          ...
          <div className="chat-messages">
            {messages.length ? (
              ...
              <p className="chat-empty">
                Faça uma pergunta sobre os documentos deste processo.
              </p>
            )}
          </div>
        ...
        <section className="process-completion-panel">
          <div>
            ...
            <p>
              {process.status === "CONCLUIDO"
                ? "Os documentos continuam disponíveis para consulta."
                : "Finalize quando terminar a revisão. Você poderá reabrir depois."}
            </p>
          </div>
          <button
            className={
      ...
      {viewer && (
        ...
        >
          <section
            aria-labelledby="process-viewer-title"
            aria-modal="true"
          ...
          >
            <div className="pdf-viewer-heading">
              <h2 id="process-viewer-title">{viewer.document.originalName}</h2>
              <button
