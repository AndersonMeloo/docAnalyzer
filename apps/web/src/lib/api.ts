...
export function getDocumentUrl(processId: string, documentId: string): string {
  return `${apiUrl}/processes/${processId}/documents/${documentId}/view`;
}
...
export async function fetchDocumentBlob(
  ...
  const response = await fetch(getDocumentUrl(processId, documentId), {
    credentials: "include",
  });
...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (reason instanceof DOMException && reason.name === "AbortError") {
      throw new Error(
        "A API demorou para responder. Verifique se o backend está ativo.",
      );
    }
    throw new Error(
...
export function login(email: string, password: string): Promise<ApiMessage> {
  return request<ApiMessage>("/auth/login", {
    method: "POST",
...
export function completeProcess(processId: string): Promise<ProcessSummary> {
  return request<ProcessSummary>(`/processes/${processId}/complete`, {
    method: "PATCH",
  });
}

export function reopenProcess(processId: string): Promise<ProcessSummary> {
  return request<ProcessSummary>(`/processes/${processId}/reopen`, {
    method: "PATCH",
  });
}
...
export function uploadDocuments(
  ...
  return request<DocumentSummary[]>(
    `/processes/${processId}/documents/upload`,
...
export function deleteDocument(
  ...
  return request<ApiMessage>(
    `/processes/${processId}/documents/${documentId}`,
    { method: "DELETE" },
  );
}
...
export function reprocessDocument(
  ...
  return request<ApiMessage>(
    `/processes/${processId}/documents/${documentId}/reprocess`,
    { method: "POST" },
  );
}
...
export function refreshProcessSummary(
  processId: string,
): Promise<ProcessSummaryResult> {
  return request<ProcessSummaryResult>(
    `/processes/${processId}/summary/refresh`,
    { method: "POST" },
  );
}

export function listFindings(processId: string): Promise<FindingSummary[]> {
  return request<FindingSummary[]>(`/processes/${processId}/findings`);
}
export function updateFinding(
  ...
  return request<FindingSummary>(
    `/processes/${processId}/findings/${findingId}`,
...
export function getConversation(
  ...
  return request<ConversationSummary>(`/processes/${processId}/conversation`);
}
export function sendMessage(
  ...
  return request<ConversationMessage>(
    `/processes/${processId}/conversation/messages`,
    { method: "POST", body: JSON.stringify({ content }) },
  );
}
export function clearConversation(processId: string): Promise<ApiMessage> {
  return request<ApiMessage>(`/processes/${processId}/conversation`, {
    method: "DELETE",
  });
}
